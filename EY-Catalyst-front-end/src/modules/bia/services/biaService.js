...
/**
 * Dummy API for critical vendor details
 * @returns {Promise} - Promise with critical vendor data
 */
export async function getCriticalVendors() {
  const token = localStorage.getItem('access_token');
  // Try both URLs if needed
  let res = await fetch(`http://localhost:8000/bia/critical-processes`, {
    headers: { Authorization: `Bearer ${token}` }
  });
  if (!res.ok && res.status === 404) {
    // Try with /api prefix if not found
    res = await fetch(`http://localhost:8000/api/bia/critical-processes`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  }
  if (!res.ok) {
    const errorText = await res.text();
    console.error('Critical processes fetch error:', res.status, errorText);
    throw new Error('Failed to fetch critical processes');
  }
