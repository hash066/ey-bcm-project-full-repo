...
const checkAPIAvailability = async () => {
  try {
    const response = await fetch(`${API_URL}/api/recovery-strategies/test`);
    return response.ok;
  } catch {
    return false;
  }
...
const getDepartmentStats = (department) => {
  ...
  const strategyTypes = { people: 0, technology: 0, site: 0, vendor: 0, technology_unavailability: 0, third_party_unavailability: 0 };
  department.sub_departments.forEach(sd => {
    sd.functions.forEach(func => {
      if (func.recovery_strategies && func.recovery_strategies.length > 0) {
        ...
        if (strategy.technology_unavailability_strategy) strategyTypes.technology_unavailability++;
        if (strategy.third_party_unavailability_strategy) strategyTypes.third_party_unavailability++;
      }
    });
  });
...
const getSubDepartmentStats = (subDepartment) => {
  ...
  const riskCategories = ['people', 'technology', 'site', 'vendor', 'technology_unavailability', 'third_party_unavailability'];
  ...
  useEffect(() => {
    const fetchRecoveryData = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/api/recovery-strategies/departments/hierarchy`);
        
        if (response.ok) {
          const data = await response.json();
          console.log("✅ Successfully fetched from API:", data);
          console.log(`📊 Loaded ${data.length} departments with real data`);
          setDepartments(data);
          setError(null);
          setLoading(false);
          return;
                ...
                functions: [
                  {
                    ...
                    recovery_strategies: [
                      {
                        process_id: "proc-1",
                        people_unavailability_strategy: "Cross-train team members and establish backup personnel for critical server management tasks",
                        people_reasoning: "Ensures continuity when key personnel are unavailable during critical incidents",
                        technology_data_unavailability_strategy: "Implement automated monitoring, backup systems, and redundant server infrastructure",
                        technology_reasoning: "Prevents data loss and system downtime through proactive monitoring and failover systems",
                        site_unavailability_strategy: "Establish remote server management capabilities and alternate data center locations",
                        site_reasoning: "Maintains operations during site disruptions or natural disasters",
                        third_party_vendors_unavailability_strategy: "Maintain relationships with multiple hardware vendors and cloud service providers",
                        vendor_reasoning: "Reduces dependency on single vendors for critical infrastructure components",
                        process_vulnerability_strategy: "Implement continuous security monitoring, regular vulnerability assessments, and incident response procedures",
                        process_vulnerability_reasoning: "Technology processes are vulnerable to cyber threats and system failures requiring proactive security measures",
                        people_status: "In Progress",
                        technology_status: "Implemented",
                        ...
                        ai_generated_sections: JSON.stringify({
                          ai_insights: {
                            risk_score: 7.5,
                          ...
                          vulnerability_analysis: [{
                            ...
                            risk_level: "high",
                            description: "Server infrastructure vulnerable to hardware failures",
                            mitigation_strategy: "Implement redundant systems and monitoring"
                          }]
                        })
                      }
                    ]
                  },
                  {
                    ...
                    recovery_strategies: [
                      {
                        process_id: "proc-2",
                        people_unavailability_strategy: "Train multiple network engineers and establish 24/7 on-call rotation",
                        people_reasoning: "Network issues require immediate attention and specialized knowledge",
                        technology_data_unavailability_strategy: "Deploy redundant network paths and automated failover systems",
                        technology_reasoning: "Network redundancy prevents single points of failure",
                        site_unavailability_strategy: "Implement distributed network management from multiple locations",
                        site_reasoning: "Network can be managed remotely during site unavailability",
                        third_party_vendors_unavailability_strategy: "Contract with multiple ISPs and network equipment vendors",
                        vendor_reasoning: "Multiple vendors ensure network connectivity alternatives",
                        process_vulnerability_strategy: "Implement network security monitoring, intrusion detection, and regular security audits",
                        process_vulnerability_reasoning: "Network infrastructure is vulnerable to cyber attacks and requires continuous monitoring",
                ...
                functions: [
                  {
                    ...
                    recovery_strategies: [
                      {
                        process_id: "proc-3",
                        people_unavailability_strategy: "Maintain comprehensive documentation and cross-train developers on multiple projects",
                        people_reasoning: "Development knowledge should not be siloed to individual developers",
                        technology_data_unavailability_strategy: "Use distributed version control and cloud-based development environments",
                        technology_reasoning: "Code and development environments must be accessible from anywhere",
                        site_unavailability_strategy: "Enable full remote development capabilities with VPN and cloud resources",
                        site_reasoning: "Development work can continue from any location with proper tools",
                        third_party_vendors_unavailability_strategy: "Use multiple cloud providers and maintain local development capabilities",
                        vendor_reasoning: "Avoid vendor lock-in for critical development infrastructure",
                        process_vulnerability_strategy: "Implement code security scanning, dependency management, and secure development practices",
                        process_vulnerability_reasoning: "Development processes are vulnerable to security flaws and dependency issues",
                        people_status: "In Progress",
                        technology_status: "Implemented",
                ...
                functions: [
                  {
                    ...
                    recovery_strategies: [
                      {
                        process_id: "proc-4",
                        people_unavailability_strategy: "Train multiple recruiters on all open positions and maintain detailed candidate databases",
                        people_reasoning: "Recruitment processes should not be dependent on individual recruiters",
                        technology_data_unavailability_strategy: "Use cloud-based ATS systems with regular backups and mobile access",
                        technology_reasoning: "Candidate data and recruitment workflows must be accessible remotely",
                        site_unavailability_strategy: "Conduct virtual interviews and remote recruitment processes",
                        site_reasoning: "Modern recruitment can be conducted entirely remotely",
                        third_party_vendors_unavailability_strategy: "Partner with multiple recruitment agencies and job boards",
                        vendor_reasoning: "Multiple channels ensure continuous talent pipeline",
                        process_vulnerability_strategy: "Implement background checks, data protection protocols, and succession planning",
                        process_vulnerability_reasoning: "HR processes handle sensitive data and are vulnerable to privacy breaches",
                ...
                functions: [
                  {
                    ...
                    recovery_strategies: [
                      {
                        process_id: "proc-5",
                        people_unavailability_strategy: "Cross-train accounting staff on all reporting processes and maintain updated procedures",
                        people_reasoning: "Financial reporting has strict deadlines and regulatory requirements",
                        technology_data_unavailability_strategy: "Implement automated backups and cloud-based accounting systems",
                        technology_reasoning: "Financial data integrity is critical for business operations",
                        site_unavailability_strategy: "Enable secure remote access to financial systems and data",
                        site_reasoning: "Financial operations can continue remotely with proper security measures",
                        third_party_vendors_unavailability_strategy: "Maintain relationships with multiple accounting software vendors and auditors",
                        vendor_reasoning: "Financial operations require reliable vendor support for compliance",
                        process_vulnerability_strategy: "Implement segregation of duties, fraud detection, and regulatory compliance monitoring",
                        process_vulnerability_reasoning: "Financial processes are vulnerable to fraud and regulatory violations",
                        people_status: "In Progress",
                        technology_status: "Implemented",
                        site_status: "In Progress",
                        vendor_status: "Implemented",
                  ...
                  },
                  {
                    ...
                    recovery_strategies: [
                      {
                        process_id: "proc-6",
                        people_unavailability_strategy: "Train backup payroll administrators and maintain detailed process documentation",
                        people_reasoning: "Payroll processing is time-sensitive and affects all employees",
                        technology_data_unavailability_strategy: "Use cloud-based payroll systems with automated backups and redundancy",
                        technology_reasoning: "Payroll data must be protected and accessible for timely processing",
                        site_unavailability_strategy: "Enable remote payroll processing with secure access controls",
                        site_reasoning: "Payroll can be processed remotely with proper security measures",
                        third_party_vendors_unavailability_strategy: "Contract with backup payroll service providers and maintain manual processes",
                        vendor_reasoning: "Payroll processing cannot be delayed due to vendor issues",
                        process_vulnerability_strategy: "Implement payroll security controls, audit trails, and backup processing capabilities",
                        process_vulnerability_reasoning: "Payroll processing is time-sensitive and affects all employees, vulnerable to errors and delays",
                        people_status: "Implemented",
                        technology_status: "Implemented",
                        site_status: "Implemented",
                        vendor_status: "In Progress",
                        process_vulnerability_status: "Implemented",
                        enabled_strategies: "people,technology,site,vendor,process_vulnerability"
                      }
                    ]
                  }
    ...
    case 'stats':
      return (
        <div className="dashboard-container" style={{ ...fullPageStyle, flex: 1, paddingTop: 0, paddingBottom: 0 }}>
          {error && (
            <div style={{background: '#FF6B35', color: '#fff', padding: '10px', borderRadius: '5px', marginBottom: '20px', textAlign: 'center'}}>
              {error}
            </div>
          )}
