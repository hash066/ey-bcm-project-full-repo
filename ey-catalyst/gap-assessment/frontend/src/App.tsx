...
import { Control, DashboardStats, Framework } from './types';
...
function App() {
  ...
  const currentControls = realControls || [];
  ...
  const filteredControls = useMemo(() => {
    return currentControls.filter(control => {
      ...
      const categoryFrameworkIds = categoryFrameworks.map(f => f.id);
  ...
  const handleFrameworkUploadComplete = (newFramework: Framework) => {
    setAllFrameworks(prev => [...prev, newFramework]);
  ...
  // Generate real action plans based on control data
  ...
  const selectedClauseMappings = useMemo(() => {
    ...
    legalAspects.forEach((aspect, index) => {
      ...
      clauses.push({
        ...
        keyStrengthOrGap,
        difficulty,
        comment: compliancePercent >= 60 ? 'Compliant based on current assessment' : 'Requires additional implementation effort'
      });
    });
  ...
  return (
    <div className="min-h-screen bg-black">
        ...
        <div className="max-w-[1920px] mx-auto px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-yellow-400">Business Continuity Portal</h1>
                <p className="text-zinc-400 mt-1">
                  Gap Assessment Dashboard
                  {jobId && <span className="ml-2 text-xs text-yellow-400">• Session: {jobId.substring(0, 8)}...</span>}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              {jobId && (
                <button
                  onClick={handleReset}
                  className="px-4 py-2 bg-zinc-800 text-white font-medium rounded-lg hover:bg-zinc-700 transition-colors flex items-center gap-2"
                >
      ...
      {/* Upload Status Toast */}
      {uploadStatus && (
        <div className="fixed bottom-6 right-6 bg-zinc-900 border border-zinc-800 rounded-lg p-4 shadow-2xl max-w-sm z-50">
          <div className="flex items-center gap-3">
