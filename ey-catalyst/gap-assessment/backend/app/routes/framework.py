...
from app.auth import get_current_active_user, check_role_hierarchy
from app.models import Framework, User, ROLE_DEPARTMENT_HEAD, ROLE_ORGANIZATION_HEAD, ROLE_EY_ADMIN
from app.schemas import Framework as FrameworkSchema, FrameworkCreate, PaginatedResponse
...
async def create_framework(
    ...
    """
    # Only department heads and above can create frameworks
    if not check_role_hierarchy(current_user, ROLE_DEPARTMENT_HEAD):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Insufficient permissions to create frameworks"
        )
