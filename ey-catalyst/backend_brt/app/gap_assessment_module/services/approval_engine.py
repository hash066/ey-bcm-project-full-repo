...
# from ...auth import check_approval_permission  # Not available, using centralized check now
from app.config_settings.role_mapping import (
    get_approval_role, can_user_approve, get_user_permissions,
    get_approval_chain, get_main_app_roles_for_approval_role, ApprovalRole, MAIN_APP_ROLE_MAPPING,
    get_next_approver_role, ROLE_HIERARCHY_LEVELS
)  # Now using centralized RBAC implementation

def check_approval_permission(user: User, required_role: str) -> bool:
    """
    Check if user has approval permission for a role.
    Now using centralized role hierarchy.

    Args:
        user: User to check permissions for
        required_role: Role that requires approval

    Returns:
        True if user has permission
    """
    user_approval_role = get_approval_role(user.role)
    required_approval_role = get_approval_role(required_role)

    if not user_approval_role or not required_approval_role:
        return False

    user_level = ROLE_HIERARCHY_LEVELS.get(user_approval_role, 0)
    required_level = ROLE_HIERARCHY_LEVELS.get(required_approval_role, 0)

    return user_level >= required_level
# from app.schemas import ApprovalRequestCreate  # Not available, will use dict instead
