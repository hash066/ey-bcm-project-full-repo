...
class GapCalculator:
    ...
    def calculate_gaps(self, extracted_data: Dict[str, Any], frameworks: Optional[List[str]] = None) -> List[GapAnalysisResult]:
        ...
        # LIMIT TEXT LENGTH TO PREVENT PROCESSING HANGS
        max_text_length = 50000  # Limit to 50k characters to prevent memory issues
        if len(text_content) > max_text_length:
            print(f"DEBUG: Truncating text from {len(text_content)} to {max_text_length} characters")
            text_content = text_content[:max_text_length] + "...[TRUNCATED DUE TO LENGTH]"
        ...
        # Determine which frameworks to use - limit to prevent excessive processing
        ...
        max_results = 100  # Limit number of results to prevent memory issues
        ...
        for framework_name in frameworks_to_check:
            ...
            for domain_name, controls in framework.domains.items():
                ...
                for control in controls:
                    ...
                    if total_controls_processed > 500:  # Limit control processing
                        print(f"DEBUG: Stopping after {total_controls_processed} controls to prevent timeout")
                        break

                    if len(results) >= max_results:  # Limit results count
                        print(f"DEBUG: Stopping after {max_results} results to prevent memory issues")
                        break
                ...
                # Check limits after domain processing
                if len(results) >= max_results or total_controls_processed > 500:
                    break

            # Check limits after framework processing
            if len(results) >= max_results or total_controls_processed > 500:
                break
