...
from ..utils.gap_calculator import GapCalculator
...
async def process_files_background(job_id: str, file_paths: List[str], original_filenames: List[str]):
    ...
    try:
        ...
        # For framework addition workflow, we only validate and store files
        # Processing happens AFTER admin approval
        for i, (file_path, original_filename) in enumerate(zip(file_paths, original_filenames)):
            try:
                logger.info(f"Validating file {i+1}/{total_files}: {original_filename}")

                # Update progress
                progress = int((i + 1) / total_files * 100)  # Complete immediately for file upload
                job_status[job_id].update({
                    "progress": progress,
                    "message": f"File {original_filename} uploaded successfully"
                })

                # Just validate file type (don't process yet)
        ...
        # Prepare upload complete result (no processing yet)
        final_result = {
            "job_id": job_id,
            "uploaded_at": datetime.utcnow().isoformat(),
            "files_uploaded": len([f for f in processed_files if "error" not in f]),
            "total_files": total_files,
            "uploaded_files": processed_files,
            "processing_status": "awaiting_approval"  # Processing happens after admin approval
        }
        ...
        job_status[job_id].update({
            "status": "complete",
            "progress": 100,
            "message": f"Successfully uploaded {len(processed_files)} files for framework addition",
            "result": final_result,
