...
from app.middleware.auth import get_current_user as get_current_active_user  # Use main app auth
from ..middleware.approval_integration import ApprovalIntegrationMiddleware
from ..services.approval_engine import ApprovalWorkflowEngine
from ...models import User  # Use main app User model
from ..models import (
    ROLE_PROCESS_OWNER, ROLE_DEPARTMENT_HEAD, ROLE_ORGANIZATION_HEAD, ROLE_EY_ADMIN, ROLE_BCM_COORDINATOR,
    APPROVAL_STATUS_PENDING, APPROVAL_STATUS_APPROVED, APPROVAL_STATUS_REJECTED,
    REQUEST_TYPE_CLAUSE_EDIT, REQUEST_TYPE_FRAMEWORK_ADDITION, Framework
)
from app.models import (  # Import approval models from main app
    ApprovalRequest, ApprovalStep
)
from app.schemas import (
    ApprovalRequestCreate, ApprovalActionRequest
)
from ..schemas import (
    ApprovalRequest, ApprovalStep, ApprovalStepCreate,
    ClauseEditRequest, FrameworkAdditionRequest,
...
def get_next_approver_role(current_role: str) -> str:
    ...
    role_hierarchy = {
        ...
        ROLE_BCM_COORDINATOR: ROLE_EY_ADMIN,  # BCM Coordinator can approve at organization level
...
async def submit_framework_addition(
    ...
    response: Response,
    # Temporarily disabled auth for testing - TODO: fix global auth setup
    # current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    ...
    # Mock current user object to avoid UserRole model conflicts
    class MockUser:
        def __init__(self):
            self.id = 1
            self.role = "CEO"
            self.department = "Executive"
            self.organization = "EY"
            self.username = "ceo_test"
            self.email = "ceo@test.com"
    ...
    # Disabled approval check for testing
    ...
    # Note: We're not validating file existence since upload stores files asynchronously
    # The file path is just a reference for documentation purposes
    pass

    # Create approval request with document path and description
    payload = {
    ...
    approval_request = create_approval_request(
        ...
        title=f"Framework Addition Request: {request.description}",
        payload=payload,
        submitted_by=current_user,
        db=db
    )
...
async def get_approval_requests(
    ...
    # Base filter: Users can see requests they submitted or requests pending their approval
    base_filter = or_(
        ApprovalRequest.submitted_by == current_user.id,
        ApprovalRequest.current_approver_role == current_user.role
    )

    # Department-scoped visibility for Department Heads and below
    if current_user.role in [ROLE_PROCESS_OWNER, ROLE_DEPARTMENT_HEAD]:
        ...
        # Include department-scoped requests
        department_filter = and_(
            ApprovalRequest.submitted_by.in_(department_users),
            ApprovalRequest.current_approver_role == current_user.role
        )

        query = query.filter(or_(base_filter, department_filter))
    else:
        # Organization Head and above can see all requests
...
async def approve_request(
    ...
    if action.decision == "approved":
        ...
        if next_role == current_user.role:  # Final approval
            ...
            # If this is a framework addition request, just create the framework (no PDF processing)
            if request.type == REQUEST_TYPE_FRAMEWORK_ADDITION:
                try:
                    # Get framework details from the approval request
                    document_file = request.payload.get('document_file')
                    description = request.payload.get('description', '')

                    if not description:
                        raise HTTPException(
                            status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Framework description is required"
                        )

                    # Create simple framework entry (no PDF processing needed)
                    framework_name = description  # Use description as name
                    framework_version = "1.0"

                    # Simple framework content (no complex PDF parsing)
                    framework_content = {
                        "description": description,
                        "source_document": document_file or "uploaded_file",
                        "created_at": datetime.utcnow().isoformat(),
                        "extraction_completed_at": datetime.utcnow().isoformat(),
                        "extracted_data": {"text": "Framework approved without processing", "controls": [], "domains": []},
                        "controls": [],
                        "domains": [],
                        "total_controls": 0,
                    ...
                    if not existing_framework:
                        # Create the new framework in the database (simple version)
                        ...
                        print(f"Framework '{new_framework.name}' created successfully (no PDF processing).")
                    else:
                        print(f"Framework '{framework_name}' already exists - skipping creation.")
...
async def get_dashboard_stats(
    ...
    from ...auth import check_role_hierarchy
    total_users = 0
...
async def approve_request_integration(
    ...
    if action.decision == "approved":
        ...
        # Disabled for testing - use mock logic
        next_approval_role = "department_head" if user_role == "process_owner" else user_role
        if next_approval_role:
            ...
            current_level = role_levels.get(next_approval_role, 0)
            ...
            if not has_higher_approvers:
                ...
                for role, level in role_levels.items():
                    if level > current_level:
                        ...
                        # Disabled for testing - just approve
                        request.current_approver_role = user_role
                        break
        else:
            request.status = APPROVAL_STATUS_APPROVED
...
async def get_user_permissions_integration(
    ...
    """
    # Disabled for testing - return mock permissions
    permissions = {"can_view_dashboard": True, "can_submit_clause_edits": True}

    return {
        "user_role": user_role,
        "approval_role": "process_owner",  # Mock
        "permissions": permissions,
        "can_approve_for_roles": ["department_head", "organization_head"]  # Mock
    }
