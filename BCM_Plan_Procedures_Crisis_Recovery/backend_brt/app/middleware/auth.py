...
# Allow optional tokens in development to avoid automatic 401s when missing
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/auth/token", auto_error=False)

def create_secure_ad_connection(server_uri, user, password, auto_bind=True):
    """
    Create a secure LDAP connection with proper SSL handling for self-signed certificates.
        ...
        try:
            ...
            try:
                ...
                if not admin_user or not admin_password:
                    raise ValueError("Missing AD admin credentials in environment")
                ...
                try:
                    ...
                    for bind_format in bind_formats:
                        ...
                        try:
                            user_conn = create_secure_ad_connection(self.server_uri, bind_user, password, auto_bind=False)
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # Dev-friendly bypass when token is missing or invalid and DEBUG is enabled
    ...
    try:
        ...
        if username is None:
            if getattr(settings, "DEBUG", False):
                # Fallback to first available user in dev
                try:
                    from app.models.rbac_models import User
                    user = db.query(User).first()
                    if user:
...
class RBACMiddleware:
    ...
    async def authenticate_user(self, username: str, password: str, db: Session) -> Optional[Dict[str, Any]]:
        ...
        Falls back to database authentication if AD is not available.
        ...
        # If AD authentication fails, try database authentication as fallback
        if not user_info:
            print(f"AD authentication failed for {username}, trying database fallback...")
            from sqlalchemy import text
            import bcrypt
            
            # Get user from database
            result = db.execute(
                text("SELECT id, username, email, hashed_password FROM users WHERE username = :username"),
                {"username": username}
            )
            user_row = result.fetchone()
            
            if user_row and user_row[3]:  # Check if user exists and has password
                # Verify password
                try:
                    if bcrypt.checkpw(password.encode('utf-8'), user_row[3].encode('utf-8')):
                        ...
                        # Return user info in same format as AD
                        user_info = {
                            "username": user_row[1],
                            "email": user_row[2],
                            "groups": role_names,  # Use roles as groups
                            "path": {}
                        }
                    else:
                        print(f"Database password verification failed for {username}")
                        return None
                except Exception as e:
                    print(f"Error verifying password: {str(e)}")
                    return None
            else:
                print(f"User {username} not found in database or has no password")
                return None
