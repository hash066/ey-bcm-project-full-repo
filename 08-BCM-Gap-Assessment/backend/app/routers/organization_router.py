    ...
    try:
        ...
        for org in organizations:
            ...
            org_data = {
                ...
                "head_username": org.get("head_username"),
                "description": org.get("description", {}),
                "impact_matrix": org.get("impact_matrix", {}),
                "licensed_modules": org.get("licensed_modules", []),
                "criticality": org.get("criticality")
            }
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Error searching organizations: {str(fallback_error)}"
            )
...
async def get_organization(
    ...
    try:
        ...
        org_data = {
            ...
            "head_username": organization.get("head_username"),
            "description": organization.get("description", {}),
            "impact_matrix": organization.get("impact_matrix", {}),
            "licensed_modules": organization.get("licensed_modules", []),
            "criticality": organization.get("criticality")
        }
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error retrieving organization: {str(fallback_error)}"
            )
...
async def get_organization_sector(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error retrieving organization sector: {str(fallback_error)}"
            )
...
async def list_organizations(
    ...
    Migrated to use Supabase as primary database with SQLAlchemy fallback.
    """
    import logging
    logger = logging.getLogger(__name__)
    ...
    try:
        # Try Supabase first
        logger.info("🔄 Attempting to fetch organizations from Supabase...")
        organizations = supabase_select(
            "global_organizations",
            limit=limit
        )
        ...
        if organizations:
            logger.info(f"✅ Retrieved {len(organizations)} organizations from Supabase")
            # Convert Supabase response to match expected format
            result = []
            for org in organizations:
                # Ensure org is a dict and has required fields
            ...
            logger.warning("⚠️ Supabase returned no data, using fallback")
            raise Exception("No data from Supabase")

    except Exception as e:
        logger.warning(f"⚠️ Supabase unavailable ({e}), falling back to local database")
        # Fallback to SQLAlchemy
        try:
            with get_db_context() as db:
                ...
                logger.info(f"✅ Retrieved {len(orgs)} organizations from local database")
                return orgs
        except Exception as fallback_error:
            logger.error(f"❌ Both Supabase and local database failed: {fallback_error}")
            raise HTTPException(
                status_code=500,
                detail="Database services are currently unavailable"
            )
...
async def get_organization_modules(
    ...
    """
    # DEVELOPMENT BYPASS: Always return all modules as licensed for BIA access
    print("=== DEVELOPMENT BYPASS: Returning mock licensed modules including BIA ===")

    # Return mock data with all important modules as licensed
    mock_licensed_modules = [
        ModuleLicense(module_id=1, is_licensed=True),
        ModuleLicense(module_id=2, is_licensed=True),
        ModuleLicense(module_id=3, is_licensed=True),  # BIA licensed
        ModuleLicense(module_id=4, is_licensed=True),
        ModuleLicense(module_id=5, is_licensed=True)
    ]

    return OrganizationModules(modules=mock_licensed_modules)
