...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // First try demo login (allows admin/demo123 for testing)
      let response, data;

      try {
        console.log('Trying demo admin login...');
        response = await fetch('http://localhost:8000/auth/simple-demo', {
          method: 'POST',
          headers: {
        ...
        if (response.ok && data.user?.roles?.[0] === 'ey_admin') {
        ...
        console.log('Demo login failed, trying AD admin login...');
      }

      // If demo login fails or doesn't return ey_admin, try AD admin authentication
      console.log('Trying AD admin login...');
      response = await fetch('http://localhost:8000/auth/token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
