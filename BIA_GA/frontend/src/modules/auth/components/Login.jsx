...
const LoginForm = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Check if user is an administrator (ey_admin role) and redirect accordingly
      const userRole = userData.user?.roles?.[0] || '';
      if (userRole === 'ey_admin') {
        console.log('Redirecting admin user to admin panel');
        navigate('/admin');
      } else {
        console.log('Redirecting regular user to home dashboard, role:', userRole);
        navigate('/home');
      }
    } catch (err) {
  ...
  return (
    <div className="auth-page">
      <div className="auth-container">
        ...
        {/* Temporarily disabled OAuth for demo testing */}
        {/*
