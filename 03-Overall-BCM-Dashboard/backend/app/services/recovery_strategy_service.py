    ...
    async def get_all_departments_with_strategies(db: Session) -> List[DepartmentWithStrategies]:
        ...
        departments = (
            ...
            .options(
                ...
                .joinedload(GlobalProcess.bia_info)
                .joinedload(BIAProcessInfo.recovery_strategy)
            )
            .all()
        )
        ...
        for dept in departments:
            ...
            for sub_dept in dept.subdepartments:
                ...
                bia_processes_in_subdept = [p.bia_info for p in sub_dept.processes if p.bia_info]
                
                for bia_process in bia_processes_in_subdept:
                    if not bia_process:
                        continue
